...
v5:Variable to capture temperature for testing purposes.
Add RAG system with performance monitoring

Feature         |    Change from V4 |
Overall Goal    |    Added a variable to capture temperature for testing purposes.
Configuration   |    Introduced the global configuration variable temperature_used = 0.2.
Prompt Building |    The default max_tokens limit in build_prompt_within_token_limit was increased from 3000 (implied usage in V4 description) to 8000.
Query Logic     |    query_index now prints the Temperature Used and uses the global temperature_used variable when setting the options for the chat function.
...
#max_tokens=3000
def build_prompt_within_token_limit(context_chunks, question, max_tokens=8000):  # Max should be upto 16K
    separator = "\n\n---\n\n"
...
def query_index(
    ...
    try:
        ...
        print(f"Temperature Used: {temperature_used}") # New code in V4 to print temperature value used
        ...
        resp = chat(
            ...
            messages=[{"role": "user", "content": prompt}],
            options={
                "temperature": temperature_used,  # Controlled temperature for factual answers #Made it temperature_used for testing purpose.
