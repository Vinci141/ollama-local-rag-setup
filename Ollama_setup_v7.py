...
"""
🚀 OPTIMIZED RAG System - Version 8 (Performance Enhanced)
...
Optional GPU: pip install torch torchvision torchaudio --index-url https://download.pytorch.org/whl/cu118

Performance Optimization
Massive speed improvements by decoupling from Ollama for utility functions: Switched Embeddings to Sentence-Transformers (10-50x faster). Switched PDF library to PyMuPDF (fitz) (15-66x faster). Switched Keyword search to BM25S (500x faster than BM25Okapi). Switched Re-ranker model to TinyBERT-L2-v2 (9x faster).
...
EMBED_MODEL_NAME = "mixedbread-ai/mxbai-embed-large-v1"  # Sentence-Transformers model
TEXT_MODEL = "gemma3"  # Ollama model for generation
RERANKER_MODEL_NAME = "cross-encoder/ms-marco-TinyBERT-L2-v2"  # Faster re-ranker (9x speedup)
...
# Batch Processing
BATCH_SIZE = 64  # Increased from 16 for better vectorization
...
print(f"\n{'=' * 70}")
print(f"🚀 RAG System - Optimized Configuration")
print(f"{'=' * 70}")
...
# ============================================================================
# HYBRID RETRIEVER (BM25S + FAISS + Cross-Encoder)
# ============================================================================
...
class HybridRetriever:
    ...
    def query(self, question: str, embedding_fn, top_k: int = TOP_K_DEFAULT) -> List[str]:
        ...
        bm25_results, bm25_scores = self.bm25.retrieve(query_tokens, k=top_k * 2)

        # FAISS semantic search
        q_embedding = embedding_fn([question])[0].reshape(1, -1)
        distances, indices = self.faiss_index.search(q_embedding, top_k * 2)
        ...
        for idx in indices[0]:
            if idx < len(self.metadata):
                ...
                candidate_docs[idx] = self.metadata[idx]["text"]

        for result_row in bm25_results[0]:
            for idx, doc_text in enumerate(self.metadata):
                if doc_text["text"] in result_row and idx < len(self.metadata):
                    candidate_set.add(idx)
                    candidate_docs[idx] = self.metadata[idx]["text"]

        candidates = [candidate_docs[idx] for idx in list(candidate_set)[:top_k * 2]]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="🚀 Optimized RAG System (v8 - Performance Enhanced)",
