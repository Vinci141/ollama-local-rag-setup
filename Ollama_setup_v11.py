...
USE_TOON = os.environ.get("USE_TOON", "false").lower() == "true"

# Keep json for fallback
if not USE_TOON:
    import json
...
def chunk_by_tokens(text: str, chunk_size: int = CHUNK_TOKEN_SIZE, overlap: int = CHUNK_OVERLAP_TOKENS,
                    model_name: str = "cl100k_base") -> List[str]:
    """
...
class EmbeddingService:
    ...
    def embed_batch(self, texts: List[str], batch_size: int = EMBED_BATCH) -> np.ndarray:
        ...
        if to_compute:
            computed = self.model.encode(to_compute, convert_to_numpy=True, batch_size=batch_size,
...
# -----------------------------
def create_faiss_index(vectors: np.ndarray, index_path: Path = FAISS_INDEX_PATH,
...
def index_folder(folder: str, rebuild: bool = False, recursive: bool = True, extensions: List[str] = None):
    ...
    # Persist metadata as newline-delimited JSON or TOON in V11
    with open(METADATA_JSON, "w", encoding="utf-8") as f:
        for m in chunk_meta:
            if USE_TOON:
                # TOON: UTF-8 default, ~50% more compact
                line = toon_encode(m)
            else:
...
def load_metadata_lines(metadata_path: Path = METADATA_JSON) -> List[Dict[str, Any]]:
    """
    Load line-delimited metadata into list in V11
    Supports both JSON and TOON formats based on USE_TOON flag.
    """
    if not metadata_path.exists():
        return []
    metas = []
...
def hybrid_search(query: str, k: int = 5, use_faiss: bool = True, rerank: bool = False) -> List[
