...
"""
🚀 OPTIMIZED RAG System - Version 9 (Enhanced PDF Reading) - IMPROVED

NEW FEATURES (v8.2):
✅ Robust PDF error handling with automatic repair
✅ Multiple text extraction strategies (layout-aware, blocks, dict)
✅ OCR fallback detection for scanned/image-based PDFs
✅ Page-by-page error recovery
✅ Corrupt PDF recovery using tobytes() cleanup
✅ Better text quality with layout preservation
✅ Validation checks for extracted content quality
...
IMPROVEMENTS (v8.2):
✅ Enhanced PDF reading with 5 extraction strategies
✅ Automatic PDF repair for corrupt files
✅ Better error handling and recovery
✅ OCR detection warnings for scanned PDFs
✅ Layout-aware text extraction for improved quality
"""
...
TOP_K_DEFAULT = 5
MAX_TOKENS = 16000  # Value updated from 8K to 16K in V8
...
print(f"\n{'=' * 70}")
print(f"🚀 RAG System - Enhanced PDF Reading (IMPROVED)")
print(f"{'=' * 70}")
...
def validate_extracted_text(text: str, min_length: int = 10) -> bool:
    ...
    # Check if text is mostly garbled (high ratio of special characters)
    if len(text) > 0:
        special_chars = sum(1 for c in text if not c.isalnum() and not c.isspace())
        ratio = special_chars / len(text)
        if ratio > 0.7:  # More than 70% special characters
            return False
...
def extract_pdf_with_layout(doc, page_num: int) -> str:
    ...
    try:
        ...
        # Use 'blocks' to preserve layout better
...
def check_pdf_needs_ocr(doc) -> bool:
    ...
    try:
        total_pages = len(doc)
        sample_size = min(3, total_pages)  # Check first 3 pages
        low_text_pages = 0
        ...
        for i in range(sample_size):
            ...
            if len(text) < 100:  # Less than 100 chars
                low_text_pages += 1

        # If most sampled pages have little text, might need OCR
...
def repair_and_clean_pdf(path: str) -> Optional[bytes]:
    """Attempt to repair a corrupt PDF and return cleaned bytes."""
    try:
        doc = fitz.open(path)
        # Use tobytes with cleaning and compression
...
def extract_text_from_pdf_robust(path: str) -> str:
    """
    Enhanced PDF text extraction with multiple strategies and error recovery.

    Strategies (in order):
    1. Standard extraction with layout preservation
    2. Block-based extraction
    3. Dictionary-based detailed extraction
    4. Page-by-page recovery for partial corruption
    5. Repair and retry for corrupt PDFs
    """
    doc = None
    text = ""

    try:
        # Initial check and repair detection
        fitz.TOOLS.reset_mupdf_warnings()
        doc = fitz.open(path)

        # Check if document needed repair
        if doc.is_repaired:
            print(f"  PDF was automatically repaired during opening")
            warnings = fitz.TOOLS.mupdf_warnings()
            if warnings:
                print(f"  Repair warnings: {warnings[:200]}")

        # Check if OCR might be needed
        needs_ocr = check_pdf_needs_ocr(doc)
        if needs_ocr:
            print(f"  ⚠️  WARNING: PDF appears to be scanned. Consider using OCR for better results.")
            print(f"      Install: pip install pytesseract pillow")
            print(f"      See: https://github.com/tesseract-ocr/tesseract")

        total_pages = len(doc)
        print(f"  Pages: {total_pages}", end="")

        # Strategy 1: Standard extraction with layout
        try:
            text_parts = []
            ...
            if validate_extracted_text(text):
                print(f" | Extracted: {len(text)} chars (standard mode)")
                doc.close()
                return text
        except Exception as e:
        ...
        try:
            text_parts = []
            for page_num in range(total_pages):
                page_text = extract_pdf_with_dict(doc, page_num)
                if page_text:
                    text_parts.append(page_text)
            ...
            if validate_extracted_text(text):
                print(f" | Extracted: {len(text)} chars (dict mode)")
                doc.close()
                return text
        except Exception as e:
            print(f"\n  Dictionary extraction failed: {e}")

        # Strategy 4: Page-by-page with error recovery
        print(f"\n  Attempting page-by-page extraction...")
        text_parts = []
        successful_pages = 0
        ...
        for page_num in range(total_pages):
            try:
                ...
                print(f"\n  Page {page_num + 1} failed: {e}")
                continue

        text = "\n\n".join(text_parts)

        if validate_extracted_text(text):
            print(f"  Extracted {successful_pages}/{total_pages} pages ({len(text)} chars)")
            doc.close()
            return text
        ...
        print(f"\n  Initial PDF opening failed: {e}")
        if doc:
            doc.close()

    # Strategy 5: Repair and retry
    print(f"  Attempting PDF repair and cleanup...")
    try:
        pdf_bytes = repair_and_clean_pdf(path)
        if pdf_bytes:
            ...
            if validate_extracted_text(text):
                print(f"  Successfully extracted after repair: {len(text)} chars")
                return text
    except Exception as e:
        print(f"  Repair strategy failed: {e}")

    print(f"  ❌ All extraction strategies failed for {os.path.basename(path)}")
    return ""
...
def extract_text_from_file(path: str) -> str:
    ...
    if ext in [".log"]:
        return None  # Handled separately by chunk_log_file
    ...
    # PDF files - Using enhanced robust extraction
    if ext == ".pdf":
